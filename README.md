...
| **Devtool Regression** | `devtool-regression.yml` | Aggregator that runs per-tool regression checks for developer tools installed via Yarn (`prettier`, `lint-staged`, `git-conventional-commits`) and uv (`pre-commit`, `reuse`). Replaces the previous `node-cli.yml` smoke test. |
